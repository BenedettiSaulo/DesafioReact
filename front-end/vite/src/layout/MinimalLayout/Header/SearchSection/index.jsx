...
import { getUsersByName, getAllUsers } from 'services/UserService';
...
const SearchSection = () => {
    ...
    const fetchUsers = useCallback(
        async (name = '') => {
            try {
                const users = name ? await getUsersByName(name) : await getAllUsers();
                setUsers(users);
            } catch (err) {
                console.log('Erro ao buscar usuários: ', err);

                showNotification('error', err.message || 'Ocorreu um erro ao adquirir os usuário.');
            }
        },
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => fetchUsers(value), 300);
