...
CREATE TABLE IF NOT EXISTS `desafioreact`.`tb_permissionlevel` (
  ...
  UNIQUE INDEX `id_nivelpermissao_UNIQUE` (`id_permissionlevel` ASC) VISIBLE,
  UNIQUE INDEX `tx_descricao_UNIQUE` (`tx_description` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `desafioreact`.`tb_user` (
  ...
  PRIMARY KEY (`id_user`),
  UNIQUE INDEX `id_usuario_UNIQUE` (`id_user` ASC) VISIBLE,
  UNIQUE INDEX `tx_email_UNIQUE` (`tx_email` ASC) VISIBLE,
  UNIQUE INDEX `tx_username_UNIQUE` (`tx_username` ASC) VISIBLE,
  INDEX `fk_tb_user_tb_permissionlevel_idx` (`cd_permissionlevel` ASC) VISIBLE,
